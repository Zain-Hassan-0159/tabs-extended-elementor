...
class tabsExtended_widget_elementor extends \Elementor\Modules\NestedElements\Base\Widget_Nested_Base {
	...
	public function get_title() {
		return esc_html__( 'Tabs Extended', 'elementor' );
	}
	...
	protected function register_controls() {
		...
		foreach ( \Elementor\Plugin::$instance->breakpoints->get_active_breakpoints() as $breakpoint_key => $breakpoint_instance ) {
		...
		$this->add_group_control(
			...
			[
				'name' => 'content_typography_dropdown',
				'label' => esc_html__( 'Dropdown Typography', 'elementor' ),
				'selector' => '{{WRAPPER}} #select_tabs_custom .select-selected',
			]
		);

		$this->add_control(
			'content_color_dropdown',
			[
				'label' => esc_html__( 'Content Color', 'elementor' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} #select_tabs_custom .select-selected' => 'color: {{VALUE}}',
					'{{WRAPPER}} #select_tabs_custom svg #Icon' => 'fill: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			'bg_color_dropdown',
			[
				'label' => esc_html__( 'Dropdown Background Color', 'elementor' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} #select_tabs_custom' => 'background-color: {{VALUE}}'
				],
			]
		);
		...
		$this->add_group_control(
			\Elementor\Group_Control_Typography::get_type(),
			[
				'name' => 'content_typography_dropdown_list',
				'label' => esc_html__( 'List Typography', 'elementor' ),
				'selector' => '{{WRAPPER}} .select-dropdown li',
			]
		);
		...
		$this->add_control(
			'color_dropdown_list',
			[
				'label' => esc_html__( 'List Text Color', 'elementor' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .select-dropdown li' => 'color: {{VALUE}}'
				],
			]
		);

		$this->add_control(
			'bg_color_dropdown_list',
			[
				'label' => esc_html__( 'List Background Color', 'elementor' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .select-dropdown' => 'background-color: {{VALUE}}'
				],
			]
		);

		$this->add_control(
			'color_dropdown_list_hover',
			[
				'label' => esc_html__( 'List Text Color Hover', 'elementor' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .select-dropdown li:hover' => 'color: {{VALUE}}'
				],
			]
		);

		$this->add_control(
			'bg_color_dropdown_list_hover',
			[
				'label' => esc_html__( 'List Background Color Hover', 'elementor' ),
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .select-dropdown li:hover' => 'background-color: {{VALUE}}'
				],
			]
		);
		...
		$this->add_control(
			'width_arrow',
			[
				'label' => esc_html__( 'Arrow Width', 'elementor' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'size_units' => [ 'px' ],
				'range' => [
					'px' => [
				...
				'selectors' => [
					'{{WRAPPER}} #select_tabs_custom svg' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			'height_arrow',
			[
				'label' => esc_html__( 'Arrow Height', 'elementor' ),
				'type' => \Elementor\Controls_Manager::SLIDER,
				'size_units' => [ 'px' ],
				'range' => [
					'px' => [
				...
				'selectors' => [
					'{{WRAPPER}} #select_tabs_custom svg' => 'height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			'drapdown_padding',
			[
				'label' => esc_html__( 'Dropdown Padding', 'elementor' ),
				'type' => \Elementor\Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px', '%', 'em', 'rem', 'custom' ],
				'selectors' => [
					'{{WRAPPER}} #select_tabs_custom' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					'{{WRAPPER}} .select-dropdown li' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
	...
	protected function render() {
		...
		foreach ( $tabs as $index => $item ) {
			...
			$tab_title_mobile_classes = [ 'e-n-tab-title', 'e-collapse', 'tabs_none_mobile' ];
			...
			$icon_html = \Elementor\Icons_Manager::try_get_icon_html( $item['tab_icon'], [ 'aria-hidden' => 'true' ] );
			$icon_active_html = $icon_html;

			if ( $this->is_active_icon_exist( $item ) ) {
				$icon_active_html = \Elementor\Icons_Manager::try_get_icon_html( $item['tab_icon_active'], [ 'aria-hidden' => 'true' ] );
			}
			...
			$mobile_tabs_option_html .= "\t<li data-tabNo={$tab_count}>{$tab_title}</li>";
		}
		?>
		<style>
			#select_tabs_custom{
			...
			<?php
				if($settings['breakpoint_selector'] === 'mobile'){
					?>
						@media(max-width: 767px){
							#select_tabs_custom{
								display: flex;
							}
							.tabs_none_mobile, .e-n-tabs-heading{
			...
			#select_tabs_custom {
				...
				.select-selected:focus + .select-dropdown,
				.select-selected:active + .select-dropdown {
				display: block;
				}
		...
		<div <?php $this->print_render_attribute_string( 'elementor-tabs' ); ?>>
			<div id="select_tabs_custom" class="custom-select">
				<div class="select-selected"><?php echo  $settings['tabs'][0]['tab_title'] ?></div>
				<div class="select-dropdown">
					<ul>
						<?php echo $mobile_tabs_option_html; ?>
					</ul>
					<input type="hidden" id="select_tabs" name="select_tabs" value="1" >
				</div>
				<svg class="select-icon" width="13px" height="8px" viewBox="0 0 13 8" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
					<g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
						<g id="Icon" transform="translate(6.001046, 1.169933) rotate(-45.000000) translate(-6.001046, -1.169933) translate(1.001046, -3.830067)" fill-rule="nonzero" fill="#000">
		...
		<script>
			...
			var select = document.querySelector("#select_tabs_custom");
			...
			var icon = select.querySelector(".select-icon");
			...
			options.forEach(function(option) {
				...
				document.getElementById('select_tabs').value = this.getAttribute('data-tabNo');
				document.getElementById('select_tabs').dispatchEvent(new Event('change'));
			...
			const selectTabs = document.getElementById('select_tabs');
			...
			selectTabs.addEventListener('change', (event) => {
				...
				// Get the element with data-tab="3"
				var element = document.querySelector('[data-tab="'+selectedValue+'"]');
	...
	protected function content_template() {
		...
		<style>
			...
			@media(max-width: 767px){
				.e-n-tabs-mobile #select_tabs_custom{
					display: flex;
				}
				.e-n-tabs-mobile .tabs_none_mobile, .e-n-tabs-mobile .e-n-tabs-heading, .e-n-tabs-mobile .e-n-tab-title{
					display: none !important;
				}
			}
			@media(max-width: 1024px){
				.e-n-tabs-tablet #select_tabs_custom{
					display: flex;
				}
				.e-n-tabs-tablet .tabs_none_mobile, .e-n-tabs-tablet .e-n-tabs-heading, .e-n-tabs-tablet .e-n-tab-title{
					display: none !important;
				}
			}
		...
		<div class="e-n-tabs" role="tablist" aria-orientation="vertical">
			...
			<div class="e-n-tabs-heading" role="tablist">
				...
				view.addRenderAttribute( tabWrapperKey, {
					'id': tabId,
					'class': [ 'tabs_none_mobile', 'e-n-tab-title','e-normal',hoverAnimationClass ],
					'data-tab': tabCount,
					'role': 'tab',
				...
				view.addRenderAttribute( tabTitleKey, {
					'class': [ 'e-n-tab-title-text', 'tabs_none_mobile' ],
			...
			<div id="select_tabs_custom" class="custom-select">
				<div class="select-selected">{{{ settings['tabs'][0].tab_title }}}</div>
				<div class="select-dropdown">
					<ul>
					<# _.each( settings['tabs'], function( item, index ) { 
					 tabCounter = index + 1,
					#>
					<li data-tabNo="{{{ tabCounter }}}" >{{{ item.tab_title }}}</li>
					<# }); #>
					</ul>
					<input type="hidden" id="select_tabs" name="select_tabs" value="1" >
				</div>
